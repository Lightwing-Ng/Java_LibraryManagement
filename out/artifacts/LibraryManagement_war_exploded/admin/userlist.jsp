...
<table border="1">
<tr>
<th>用户ID</th> <th>用户名字</th> <th>用户班级</th> <th>用户性别</th> <th>用户电话</th><th>用户地址</th>
</tr>
<c:forEach var="users" items="${requestScope.list}">
<tr>
<td>${users.users_ID}</td> <td>${users.users_Name}</td> <td>${users.users_Class}</td> <td>${users.users_Sex==1?"男":"女"}</td> <td>${users.users_Phone}</td><td>${users.users_Adds}</td>
</tr>
</c:forEach>
</table>
<table>
<tr>
<td><a href="UsersPageAction.do?page=${requestScope.page-1<=0?"1":requestScope.page-1}">上一页</a></td><td><a href="UsersPageAction.do?page=${requestScope.page+1>=requestScope.pagecount?requestScope.pagecount:requestScope.page+1}">下一页</a></td><td>${requestScope.page}/${requestScope.pagecount}页</td>
</tr>
</table>
</center>
